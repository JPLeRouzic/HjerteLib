...
public class AudioMethodsDSP
{
    ...
    public float[][] applyGain(float samples[][], float gain)
    {
    ...
    public void applyGainAndPanning(float samples_to_modify[][], float gain, float panning)
        throws Exception
    {
    ...
    public float[] getSamplesMixedDownIntoOneChannel(float audio_samples[][])
    {
    ...
    public float[][] clipSamples(float original_samples[][])
        throws Exception
    {
    ...
    public float[] normalizeSamples(float samples_to_normalize[])
    {
    ...
    public float[][] getCopyOfSamples(float original_samples[][])
    {
    ...
    public int convertTimeToSample(float time, float sampling_rate)
    {
        return Math.round(time * sampling_rate);
    }

    public float convertSampleToTime(int sample, float sampling_rate)
    {
        return (float)sample / sampling_rate;
    }

    public float findMaximumSampleValue(int bit_depth)
    {
    ...
    public float[] getAutoCorrelation(float signal[], int min_lag, int max_lag)
    {
    ...
    public float[] getAutoCorrelationLabels(float sampling_rate, int min_lag, int max_lag)
    {
    ...
    public void applyClickAvoidanceAttenuationEnvelope(float sample_values[][], float click_avoid_env_length, float sample_rate)
        throws Exception
    {
        ...
        {
            float amplitude_multipler = (float)(1.0D - ((samp - start_sample_2) / (end_sample_2 - start_sample_2)));
            for(int chan = 0; chan < sample_values.length; chan++)
            {
